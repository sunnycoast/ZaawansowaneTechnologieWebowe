	...
	tPMe.forEach(function(el, i) 
		{
			if(tKat[kat]["id"]==el["kat"])
			{
				...
				sqr += '<div class="cKateg", onclick="dodajProdukt('+i+')", style="background-color: #'+tKat[kat]["kol"]+';">';
				sqr += el["nazw"]+'<br/>'+el["cena"].toFixed(2)+' zł</div>';
				sqr += '<div class="cDivOp">'+el["opis"]+'</div>';
				sqr += '</div>';
				//*/
				//sqr += '<button class="cBattonZam", onclick="dodajProdukt('+i+')", style="background-color: #'+el["kol"]+';">'+el["nazw"]+'<br/>'+el["cena"].toFixed(2)+' zł '+'</button>';
				//sqr += '<div class="cDivOp">'+el["opis"]+'</div>';
			}
		});
	return sqr;
